...
function pdelete(id){
	var products=JSON.parse(localStorage.getItem("products"));
	var a=window.confirm("Si desea eliminar este producto?"+id);
	 if(a==true){
		...
		alert("Eliminación de producto fallida");
	}
}
...
function selectView(){
	...
	if(checkDisplay==true){
		...
		alert("Seleccione Fila para ver");
	}
}
...
function selectDelete(){
	...
	if(checkDisplay==true){
		localStorage.setItem("products",JSON.stringify(products));
		alert("Elementos seleccionados y eliminados");
		window.location.reload();
	}
	else{
		alert("Seleccione la fila para eliminar");
	}
}